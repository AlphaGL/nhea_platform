...
from . import views
from .views import (
    NheaLoginView, AdminLoginView, SuperadminLoginView,
    vote_category, next_category,
    admin_dashboard, superadmin_dashboard, register_voter, CompletedView,
    reset_password, verify_phone, verify_email, use_email_instead,
    voting_dashboard,
)

urlpatterns = [
    # ── Login Portal (role selector landing page) ─────────────────
    # path('portal/',            views.login_portal,            name='login_portal'),

    # ── Voter login (existing, unchanged) ─────────────────────────
    path('',                   NheaLoginView.as_view(),        name='login'),

    # ── Admin-specific login ──────────────────────────────────────
    path('custom-admin/login/',       AdminLoginView.as_view(),       name='admin_login'),
    ...
    path('panel/promote-superadmin/<int:admin_id>/',     views.promote_to_superadmin, name='promote_to_superadmin'),
